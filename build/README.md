...
node build-docs.mjs --only TD-004  # one document
...
| Artefact | Contents |
|---|---|
| `Thar Deck-CD-00N-<Title>-v<version>.pdf` | One controlled document, with cover, revision history and contents |
| `Thar Deck-Technical-Manual-v<version>.pdf` | All documents bound as one volume with part dividers |
...
A printed page cannot follow a hyperlink, so `[Audio Chain](04-audio-chain.md)` renders as the link text followed by a `TD-004` tag. The mapping comes from `docs/manifest.json`.
